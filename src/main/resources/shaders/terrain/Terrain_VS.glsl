...
vec2 morph(vec2 localPosition, float height, int morphArea) {
    ...
    distLatitude = length(cameraPosition - (worldMatrix * vec4(fixPointLatitude.x, height, fixPointLatitude.y, 1)).xyz);
    distLongitude = length(cameraPosition - (worldMatrix * vec4(fixPointLongitude.x, height, fixPointLongitude.y, 1)).xyz);

    if (distLatitude > morphArea) {
        morphing.x = morphLatitude(localPosition.xy);
    }
    if (distLongitude > morphArea) {
        morphing.y = morphLatitude(localPosition.xy);
    }
