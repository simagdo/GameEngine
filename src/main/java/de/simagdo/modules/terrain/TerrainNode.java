...
public class TerrainNode extends GameObject {
    ...
    public TerrainNode(PatchVBO patchVBO, TerrainConfig terrConfig, Vec2f location, int lod, Vec2f index) {
        ...
        getLocalTransformation().setScaling(localScaling);
        getLocalTransformation().setTranslation(localTranslation);

        getWorldTransformation().setScaling(terrConfig.getScaleXZ(), terrConfig.getScaleY(), terrConfig.getScaleXZ());
        getWorldTransformation().getTranslation().setX(-terrConfig.getScaleXZ() / 2f);
        getWorldTransformation().getTranslation().setZ(-terrConfig.getScaleXZ() / 2f);
        getWorldTransformation().getTranslation().setY(0);

        Renderer mainRenderer = new Renderer();
        mainRenderer.setVbo(buffer);
        mainRenderer.setRenderInfo(new RenderInfo(new DefaultConfig(), TerrainShader.getInstance()));
