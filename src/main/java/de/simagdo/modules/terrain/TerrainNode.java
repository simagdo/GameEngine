...
public class TerrainNode extends GameObject {
    ...
    public TerrainNode(PatchVBO buffer, TerrainConfig config, Vec2f location, int lod, Vec2f index) {
        ...
        this.getWorldTransform().setScaling(this.config.getScaleXZ(), this.config.getScaleY(), this.config.getScaleXZ());
    ...
    public void updateQuadTree() {
        this.worldPosition.setY(Math.min(Camera.getInstance().getPosition().getY(), this.config.getScaleY()));
    ...
    private void addChildNodes(int lod) {
        ...
        if (this.getChildren().size() == 0) {
            for (int i = 0; i < 2; i++) {
                for (int j = 0; j < 2; j++) {
                    this.addChild(new TerrainNode(this.buffer, this.config, this.location.add(new Vec2f(i * this.gap / 2f, j * this.gap / 2f)), lod, new Vec2f(i, j)));
                }
            }
        }
    }
    ...
    private void updateChildNodes() {
        ...
        if (distance < this.config.getLodRange()[this.lod]) this.addChildNodes(this.lod++);
