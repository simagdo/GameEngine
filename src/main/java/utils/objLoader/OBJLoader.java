...
import de.simagdo.engine.model.Model;
...
public class OBJLoader {
    ...
    private Deque<Object> objects = new ArrayDeque<>();
    ...
    public Model[] load(String path, String objFile, String mtlFile) {
        ...
        // load .obj
        try {
            String[] lines = Utils.getDataAsString(path + "/" + objFile).split("\n");
            for (String line : lines) {
                ...
                if (tokens[0].equals("v")) {
                    vertices.add(new Vertex(new Vec3f(Float.parseFloat(tokens[1]),
                            Float.parseFloat(tokens[2]),
                            Float.parseFloat(tokens[3]))));
                }
                if (tokens[0].equals("vn")) {
                    normals.add(new Vec3f(Float.parseFloat(tokens[1]),
                            Float.parseFloat(tokens[2]),
                            Float.parseFloat(tokens[3])));
                }
                if (tokens[0].equals("vt")) {
                    texCoords.add(new Vec2f(Float.parseFloat(tokens[1]), Float.parseFloat(tokens[2])));
                }
                    ...
                    for (PolygonGroup polygonGroup : object.getPolygonGroups()) {
                        for (Integer key : polygonGroup.getSmoothingGroups().keySet()) {
                            if (frontface == Frontface.CW)
                                Utils.generateNormalsCW(polygonGroup.getSmoothingGroups().get(key));
                            else
                                Utils.generateNormalsCCW(polygonGroup.getSmoothingGroups().get(key));
                        }
                    }
                }
            }
    ...
    public Model convert(Polygon polygon) {
        ...
        int[] indexData = Utils.toIntArray(objectArray);
